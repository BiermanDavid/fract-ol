...
/*   Updated: 2025/03/18 11:10:15 by dabierma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

static int	choose_fractal(t_fractal *fract, double c_r, double c_i)
{
	if (fract->type == 1)
		return (mandelbrot(c_r, c_i));
	else if (fract->type == 2)
		return (julia(fract, c_r, c_i));
	return (1);
}
